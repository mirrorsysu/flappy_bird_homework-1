...
def createPipes(layer, gameScene, spriteBird, score, level = "easy"):
    ...
    def initPipe():
        for i in range(0, pipeCount):
            ...
            downPipe = CollidableRectSprite("pipe_down", 0, (pipeHeight + pipeDistance), pipeWidth / 2,
                                            pipeHeight / 2)  # 朝下的pipe而非在下方的pipe
    ...
    def movePipe(dt):
        ...
        if level == "easy":
            speed = 2 - g_score / 20.0
            if speed <= 1.8:
                speed = 1.8
        elif level == "mid":
            heightOffset = random.randint(60,130)
            speed = 1.9 - g_score / 20.0
            if speed <= 1.4:
                speed = 1.4
        elif level == "hard":
            heightOffset = random.randint(0, 150)
            speed = 1.8 - g_score/20.0
            if speed <= 1:
                speed = 1
        moveDistance = common.visibleSize["width"] / (speed * 60)  # 移动速度和land一致
        for i in range(0, pipeCount):
